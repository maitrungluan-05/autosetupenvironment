...
function Get-JavaHomeInfo { param([string]$Value)
 ...
 [pscustomobject]@{Value=$Value;Exists=(Test-Path $Value);ValidJdk=$valid;JavaPath=$java;JavacPath=$javac;Version=$version}
}
function Get-JavaDiagnostic {
 ...
 foreach($javaHomeValue in @([Environment]::GetEnvironmentVariable('JAVA_HOME','User'),[Environment]::GetEnvironmentVariable('JAVA_HOME','Machine'))|Where-Object{$_}){$candidateHomes+=$javaHomeValue}
 ...
 $javaHomeInfo=Get-JavaHomeInfo ([Environment]::GetEnvironmentVariable('JAVA_HOME','User'));if(-not $javaHomeInfo.Value){$javaHomeInfo=Get-JavaHomeInfo ([Environment]::GetEnvironmentVariable('JAVA_HOME','Machine'))}
...
function Set-JavaHomeSafely { param([string]$Target,[switch]$Yes)
 ...
 $old=[Environment]::GetEnvironmentVariable('JAVA_HOME','User');if($old -and $old -ne $Target){if(-not(Confirm-Action 'Update JAVA_HOME?')){return @{Success=$false;Cancelled=$true}}}
 $backup=Join-Path (Get-DevSetupLogDirectory) ('JAVA_HOME-'+(Get-Date -Format yyyyMMddHHmmss)+'.bak');Set-Content $backup $old
 try{[Environment]::SetEnvironmentVariable('JAVA_HOME',$Target,'User');Refresh-ProcessEnvironment;$after=Get-JavaHomeInfo $env:JAVA_HOME;if(-not $after.ValidJdk){throw 'JAVA_HOME verification failed.'};@{Success=$true;Backup=$backup}}catch{[Environment]::SetEnvironmentVariable('JAVA_HOME',$old,'User');Refresh-ProcessEnvironment;@{Success=$false;Error=$_.Exception.Message;RolledBack=$true}}
}
